...
class electronMotion():
    def __init__(self, x, y):
        ...
        self.dt = 1e-9
        self.time = 0
        self.MagneticField = 5e-5*vector(0,-5,-5)
        self.p = self.m*self.v*vector(0,3,0)

    def magneticForce(self):
        self.force = self.charge * self.v* self.MagneticField
        
    def updateMomemtum(self):
        self.p = self.p + self.force*dt
...
def retrieve_input(x0,y0):
    ...
    v=float(textBox1.get("1.0","end-1c"))
